...
# ========== CORRECT IMPORTS ==========
# This is the correct way to import from src folder
try:
    from src.fluids.rheology import BinghamPlasticModel
except ImportError:
    # Fallback if rheology module is not found
    st.warning("Rheology module not found. Some features may not work.")
    BinghamPlasticModel = None

# These are for future modules (placeholders)
try:
    from src.cementing import volumetrics
except ImportError:
    volumetrics = None
...
try:
    from src.database import additives
except ImportError:
    additives = None
...
# ============================================
# PAGE 4: RHEOLOGY ANALYSIS (UPDATED)
# ============================================
    ...
    # Check if rheology module is available
    if BinghamPlasticModel is None:
        st.error("❌ Rheology module not available. Please check your installation.")
        st.stop()
...
st.sidebar.markdown("---")
st.sidebar.caption("Version 2.1 | August 2026")